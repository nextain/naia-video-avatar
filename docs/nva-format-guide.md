...
로컬 서버로 데모(`src/main/demo.html`)를 열면 박스 캐릭터로 **시나리오 4개가 자동 연출**된다(안내·길 안내·환영·작별).
...
| (얼굴 영역) | speaking 시 face_bbox 위치에 립싱크 — 캐스케이드 런타임이 실시간 렌더 가능 |
...
- 알파: 데모는 크로마키(ffmpeg 알파 미지원 환경 우회). 실 파이프라인은 캐스케이드 런타임이 VP9 알파 생성 — 포맷은 알파/크로마 둘 다 수용.
- speaking: 데모는 mock 입 오버레이. 캐스케이드 연결 시 실시간 립싱크(`?cascade=/avatar`).

## 7. 권리

포맷 명세 + 뷰어/에디터 = naia(nextain) 자산(과도기 오픈 표준). 번들에 담기는 캐릭터(클립·음색) = 각 제작자 자산(`meta.owner`).

...
> v0.1→v0.2 전환(animations/scenario 그래프, 캐스케이드 정합). editor = 알파 webm 렌더 +
> scenario SVG 노드그래프(드래그·연결·양방향) + 캐스케이드 연결 패널(실시간 발화 뷰잉, 상단 플레이어
> 통합, 미리보기 배경색). 캐스케이드 `enable_alpha=1` 로 발화도 투명(정적 matte).
...
| P05 완료 | partial | 포맷·뷰어·에디터·export = Done / 캐스케이드 = 잔여(GPU 외부) |

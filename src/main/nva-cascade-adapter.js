// nva ↔ 캐스케이드 런타임 연결 어댑터 (브라우저, ESM)
//
// nva talking state의 speaking을 캐스케이드 런타임의 호환 façade로 실시간 렌더한다.
// 캐스케이드가 nva 번들(캐릭터)을 로드해 음성 합성 + 립싱크 렌더로 말하는 얼굴을 fMP4로
// 스트리밍하고, 이 어댑터가 MSE로 첫 청크부터 재생한다. 캐스케이드 미연결이면 viewer는
// mock 입으로 폴백.
//
// 책임 경계:
//   - 이 어댑터(naia-video-avatar) = 브라우저측 speaking 백엔드 호출 + MSE 재생.
//   - 캐스케이드 런타임 = 음성 합성 + 립싱크 렌더, nva 번들 소비. (GPU, 별도 저장소)
//   - 키오스크/셸 앱 = /avatar 프록시 → 캐스케이드. (배포 게이트웨이)
//
// 캐스케이드 façade 계약:
...
export class NvaCascadeRenderer {
  /** @param {string} cascadeUrl 예: "/avatar" 또는 "http://localhost:<port>" @param {HTMLVideoElement} videoEl */

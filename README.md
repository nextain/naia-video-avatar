...
> **에디터(제작 도구) ↔ 데모(시연)는 분리**. 에디터는 특정 배포와 무관한 범용 nva 제작용, 데모는 만든 nva를 캐스케이드로 실행.
...
제작 도구: `src/main/editor.html` (로컬 서버로 실행)
...
실배포는 캐스케이드 런타임의 립싱크 렌더러가 VP9 yuva420p 알파 생성 — 포맷/도구는 알파·크로마 **둘 다 수용**.
